...
import com.itsmerizzi.payment_system.dto.PixChargeRequest;
import com.itsmerizzi.payment_system.pix.Credentials;
...
public class PixService {
    ...
    public JSONObject pixCreateEVP() {
        JSONObject options = configurateOptions();

        try {
            EfiPay efi = new EfiPay(options);
            return efi.call("pixCreateEvp", new HashMap<String, String>(), new JSONObject());
        } catch (EfiPayException e) {
            throw new RuntimeException("Error while creating PIX", e);
        } catch (Exception e) {
    ...
    public JSONObject pixCreateCharge(PixChargeRequest pixChargeRequest) {
        ...
        body.put("devedor", new JSONObject().put("cpf", "12345678909").put("nome", "Augusto Romano"));
        ...
        try {
            EfiPay efi = new EfiPay(options);
            JSONObject response = efi.call("pixCreateImmediateCharge", new HashMap<String, String>(), new JSONObject());

            int idFromResponse = response.getJSONObject("loc").getInt("id");
            pixGenerateQRCode(String.valueOf(idFromResponse));

            return response;
        } catch (EfiPayException e) {
            throw new RuntimeException("Error while creating charge", e);
        } catch (Exception e) {
            throw new RuntimeException("Unexpected error", e);
        }
    }
    ...
    private JSONObject configurateOptions() {
